...
const BottomBarMenu: React.FC = (): React.ReactElement => {
    ...
    return (
        <div className="absolute left-0 top-0 bottom-0 flex flex-col items-center justify-center border-r border-white/10 p-2">
            <div className="flex flex-col items-start justify-center w-full h-full">
                <button
                    type={"button"}
                    className={`w-full text-lg font-semibold text-white rounded-sm p-1 ${bottomBarView === BottomBarView.Debug ? 'bg-blue-500' : 'bg-zinc-500'}`}
                ...
                <button
                    type={"button"}
                    className={`w-full text-lg font-semibold text-white rounded-sm p-1 mt-1 ${bottomBarView === BottomBarView.Output ? 'bg-blue-500' : 'bg-zinc-500'}`}
