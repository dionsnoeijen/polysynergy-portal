...
const VariableTypeJmes: React.FC<VariableTypeProps> = ({
    nodeId,
    variable,
    ...
    const onEdit = React.useCallback((nodeId: string) => {
        useEditorStore.getState().openForm(FormType.EditJmes, nodeId, variable);
    }, [variable]);
             ...
             {isValueConnected ? (
                 ...
                 <Fieldset>
                    ...
                    <Field>
                        ...
                        >
                            <CodeBracketSquareIcon className="w-4 h-4 inline text-cyan-700/80 dark:text-cyan-400/80"/>
                        </button>
                    </Field>
                </Fieldset>
             )}
...
export default React.memo(VariableTypeJmes, (prevProps, nextProps) => {
    return (
