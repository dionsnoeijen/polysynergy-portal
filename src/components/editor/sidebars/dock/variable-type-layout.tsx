...
const VariableTypeLayout: React.FC<VariableTypeProps> = ({
    nodeId,
    variable,
    ...
    const onEdit = React.useCallback((nodeId: string) => {
        useEditorStore.getState().openForm(FormType.EditLayout, nodeId, variable);
    }, [variable]);
             ...
             {isValueConnected ? (
                 ...
                 <Fieldset>
                    ...
                    <Field>
                        ...
                        >
                            <Squares2X2Icon className="w-4 h-4 inline text-blue-700/80 dark:text-white/50"/>
                        </button>
                    </Field>
                </Fieldset>
             )}
...
export default React.memo(VariableTypeLayout, (prevProps, nextProps) => {
    return (
