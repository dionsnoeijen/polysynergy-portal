...
const VariableTypeTable: React.FC<VariableTypeProps> = ({
    nodeId,
    variable,
    ...
    const onEdit = React.useCallback((nodeId: string) => {
        useEditorStore.getState().openForm(FormType.EditTable, nodeId, variable);
    }, [variable]);
             ...
             {isValueConnected ? (
                 ...
                 <Fieldset>
                    ...
                    <Field>
                        ...
                        >
                            <TableCellsIcon className="w-4 h-4 inline text-blue-700/80 dark:text-white/50"/>
                        </button>
                    </Field>
                </Fieldset>
             )}
...
export default React.memo(VariableTypeTable, (prevProps, nextProps) => {
    return (
