...
interface PublishedVariablesResult {
  initialVariables: { [handle: string]: { [variableHandle: string]: NodeVariable[] } };
  initialSimpleVariables: { [handle: string]: { [variableHandle: string]: string } };
  variablesByHandle: { [handle: string]: { variables: NodeVariable[]; nodeIds: string[] } };
}

const findPublishedVariables = (nodes: Node[]): PublishedVariablesResult => {
